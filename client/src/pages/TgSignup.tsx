...
export interface TgSessionQueryParams {
  userId: string
  sessionId: string
}
...
const Signup = (): React.JSX.Element => {
  ...
  const verify = useCallback(async (inputCode: string): Promise<void> => {
    const { address, ekey, eseed } = utils.computeParameters({ phone, p, pk })
    const signature = apis.web3.wallet(utils.hexString(pk)).signMessageSync(hash)
    try {
      setVerifying(true)
      const success = await apis.server.verify({ phone, address, ekey, eseed, code: inputCode, signature })
      if (success) {
        ...
        toast.error('Sign up failed for unknown reason')
      }
    } catch (ex) {
      console.error(ex)
  ...
  const onSubmit = async (e): Promise<void> => {
    e.preventDefault()
    navigate(location.pathname + '#submitted')
    setTriedSave(true)
    onSaveQR()
    // onDone && onDone()
  }
